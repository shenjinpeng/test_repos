...
void COsdFxDrawWindow(WORD usXStart, WORD usYStart, WORD usXEnd, WORD usYEnd, BYTE *pStyle)
{
    ...
    // Set double buffer depth to be 6
    CScalerSetBit(_OSD_SCRAMBLE_93, ~(_BIT0 | _BIT1 | _BIT2), (_BIT2 | _BIT0));
...
void COsdFxDisableWindow(BYTE window)
{
        ...
        CScalerWrite(_OSD_ADDR_MSB_90, 2, pData, _AUTOINC);
        pData[0] = 0x00;
        CScalerWrite(_OSD_DATA_PORT_92, 1, pData, _AUTOINC);


}

void COsdFactoryReset(void)
{

stSystemData.BackLight=0x80;
if(GET_INPUTSOURCE_TYPE()==_SOURCE_VGA)
{
stConBriData.Brightness=0x7e;//_BRIGHTNESS_VGA;
stConBriData.Contrast=0x80;//_CONTRAST_VGA;
stConBriData.Dicom=00;
stConBriData.Gamma=00;
stConBriData.DicomLumi=80;
}
else
{
stConBriData.Brightness=0x80;//_BRIGHTNESS_DVI;
stConBriData.Contrast=0x80;//_CONTRAST_DVI;
stConBriData.Dicom=00;
stConBriData.Gamma=00;
stConBriData.DicomLumi=80;
...
void COsdFxSetPosition(WORD usOsdActWidth, WORD usOsdActHeight, BYTE ucHPos, BYTE ucVPos, BYTE ucPar)
{
    ...
    // Patch Reason : Modify OSD Set position and Draw Window Function
    /*
// For Bank Switch Program Modify Start
...
// For Bank Switch Program Modify End
    ...
    */
    if(usOsdActWidth > _PANEL_DH_WIDTH)
    {
        usOsdActWidth = _PANEL_DH_WIDTH;
    }

    if(usOsdActHeight > _PANEL_DV_HEIGHT)
    {
        usOsdActHeight = _PANEL_DV_HEIGHT;
    }

    // Vertical delay unit 1 line, Horizontal delay unit 4 pixels.
    usOsdActWidth = _OSD_HPOSITION_OFFSET + ((DWORD)(_PANEL_DH_WIDTH - usOsdActWidth) * ucHPos / _OSD_HPOSITION_RANGE) / 4;
    usOsdActHeight = _OSD_VPOSITION_OFFSET + (DWORD)(_PANEL_DV_HEIGHT - usOsdActHeight) * ucVPos / _OSD_VPOSITION_RANGE;

    pData[0] = 0xC0;
    pData[1] = 0x00;
    CScalerWrite(_OSD_ADDR_MSB_90, 2, pData, _AUTOINC);

    pData[0] = usOsdActHeight >> 3;
    pData[1] = (BYTE)(usOsdActWidth >> 2);
    pData[2] = ((BYTE)(usOsdActWidth & 0x03) << 6) | ((usOsdActHeight & 0x07) << 3) | ucPar;
    CScalerWrite(_OSD_DATA_PORT_92, 3, pData, _NON_AUTOINC);
...
WORD COsdFxGetAdcClockRange(BYTE ucPar)
{
    ...
    if(ucPar == _GET_CLOCKRANGE_MAX)
    {
        ...
        {
            ((WORD *)pData)[5] = (stModeInfo.IHTotal + delta);
        }
        else
        {
            if(stModeUserCenterData.CenterClock < stModeInfo.IHTotal)
            {
                ...
                {
                    ((WORD *)pData)[5] = (stModeInfo.IHTotal - delta + 2 * _CLOCK_BIAS);
                }
                else
                {
                    ((WORD *)pData)[5] = (stModeUserCenterData.CenterClock + _CLOCK_BIAS);
                }
            }
            else
            {
    ...
    if(CAutoCheckClock(((WORD *)pData)[5]) == _FALSE)
    {
        ((WORD *)pData)[5] = g_usCaptureStart + stModeInfo.IHWidth + _CAPTURE_HDELAY;
    }
    else if(((WORD *)pData)[5] > g_usAdcClockLimit)
    {
        // Capture Start + Capture Window < Clock
        if(CAutoCheckClock(g_usAdcClockLimit) == _TRUE)
        {
            ((WORD *)pData)[5] = g_usAdcClockLimit;
        }
    }
...
WORD COsdFxDetOverRange(WORD ucValue, WORD ucMax, WORD ucMin, BYTE ucSymbol)
{
    switch(ucSymbol)
    {
        case _DET_OVERRANGE_MAX:
            ...
            {
                ucValue++;
		  //if (stConBriData.Dicom<8)//sjp20110625
		//	ucValue= ucValue+26;//sjp20110625
